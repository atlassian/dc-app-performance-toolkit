...
| Confluence Version | The Data Center App Performance Toolkit officially supports `7.4.8` ([Long Term Support release](https://confluence.atlassian.com/enterprise/atlassian-enterprise-releases-948227420.html)) and `7.0.5` Platform Release |
...
1. App-specific tests could be run (if needed) as a specific user. In the `standalone_extension` uncomment `run_as_specific_user` controller. Navigate to the `username:password` config element and update values for `app_specific_username` and `app_specific_password` names with your specific user credentials. Also make sure that you located your app-specific tests between `run_as_specific_user` and `disable_specific_user_if_exists` controllers.   
