...
def insight_app_specific_action(webdriver, datasets):
    ...
    @print_timing("selenium_customer_insight_app_custom_action")
    def measure():

        @print_timing("selenium_customer_app_custom_action:view_request")
        def sub_measure():
            page.go_to_url(f"{JSM_SETTINGS.server_url}/servicedesk/customer/portal/{portal_id}")
            # Wait for options element visible
            page.wait_until_visible((By.ID, 'cv-request-content'))
