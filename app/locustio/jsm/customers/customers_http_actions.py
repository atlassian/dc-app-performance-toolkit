...
def customer_view_requests(locust):
    ...
    def customer_view_my_requests(locust):
        locust.post(
            '/rest/webResources/1.0/resources',
            json=params.resources_body.get("250"),
            headers=RESOURCE_HEADERS,
            catch_response=True)
        customer_models = {
            "models": [
        ...
        locust.post(
            '/rest/analytics/1.0/publish/bulk',
            json=params.resources_body.get("260"),
            headers=RESOURCE_HEADERS,
            catch_response=True)
    ...
    def customer_view_all_requests(locust):
        locust.post(
            '/rest/webResources/1.0/resources',
            json=params.resources_body.get("300"),
            headers=RESOURCE_HEADERS,
            catch_response=True)
        customer_models = {
            "models": [
...
def customer_add_comment(locust):
    ...
    locust.post(
        '/rest/analytics/1.0/publish/bulk',
        json=params.resources_body.get("365"),
        headers=RESOURCE_HEADERS,
        catch_response=True)
...
def customer_create_request(locust):
    ...
    def customer_open_create_request_view(locust):
        ...
        locust.post(
            '/rest/analytics/1.0/publish/bulk',
            json=params.resources_body.get("602"),
            headers=RESOURCE_HEADERS,
            catch_response=True)
...
def update_issue_summary(locust, new_summary=None):
    ...
    encoded_bytes = base64.b64encode(
        f'{locust.config.admin_login}:{locust.config.admin_password}'.encode('utf-8'))
    ...
    r = locust.client.put(
        f'/rest/api/2/issue/{locust.session_data_storage["request_key"]}',
        data=payload_json,
