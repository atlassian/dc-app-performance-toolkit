...
def view_issue(locust):
    ...
    locust.get(
        f'/secure/projectavatar?avatarId={project_avatar_id}',
        catch_response=True)
...
def create_issue(locust):
    ...
    def create_issue_open_quick_create():
        ...
        r = locust.post(
            '/secure/QuickCreateIssue!default.jspa',
            json={
                'atlassian.xsrf.token': locust.session_data_storage["token"]},
    ...
    def create_issue_submit_form():
        raise_if_login_failed(locust)
        issue_body = params.prepare_issue_body(
            locust.session_data_storage['issue_body_params_dict'],
        ...
        logger.locust_info(
            f"{params.action_name}: Issue {issue_key} was successfully created")
...
def search_jql(locust):
    ...
    locust.get(
        f'/rest/api/2/filter/favourite?expand=subscriptions[-5:]&_={timestamp_int()}',
        catch_response=True)
    ...
    if issue_ids:
        ...
        if params.edit_allow_string in r.content.decode('utf-8'):
            locust.get(
                f'/secure/AjaxIssueEditAction!default.jspa?'
                f'decorator=none&issueId={issue_id}&_={timestamp_int()}',
                catch_response=True)
...
def view_project_summary(locust):
    ...
    content = r.content.decode('utf-8')
    logger.locust_info(
        f"{params.action_name}. View project {project_key}: {content}")

    assert re.compile(
        f'.*project-key.*{project_key}').search(content), params.err_message
    ...
    locust.get(
        f'/projects/{project_key}?selectedItem=com.atlassian.jira.jira-projects-plugin:'
...
def edit_issue(locust):
    ...
    def edit_issue_open_editor():
        ...
        r = locust.get(
            f'/secure/EditIssue!default.jspa?id={issue_id}',
            catch_response=True)

        content = r.content.decode('utf-8')
        issue_type = fetch_by_re(params.issue_type_pattern, content)
        atl_token = fetch_by_re(params.atl_token_pattern, content)
        priority = fetch_by_re(
            params.issue_priority_pattern,
            content,
            group_no=2)
        assignee = fetch_by_re(
            params.issue_assigneee_reporter_pattern,
            content,
            group_no=2)
        ...
        if not (f' Edit Issue:  [{issue_key}]' in content):
            logger.error(
                f'{params.err_message_issue_not_found} - {issue_id}, {issue_key}: {content}')
        ...
        locust.get(
            f'/rest/internal/2/user/mention/search?issueKey={issue_key}'
            f'&projectKey={project_key}&maxResults=10&_={timestamp_int()}',
            catch_response=True)
    ...
    def edit_issue_save_edit():
        ...
        r = locust.post(
            f'/secure/EditIssue.jspa?atl_token={locust.session_data_storage["atl_token"]}',
        ...
        locust.get(
            f'/rest/bamboo/latest/deploy/{project_key}/{issue_key}?_{timestamp_int()}',
            catch_response=True)

        # 750 /secure/AjaxIssueEditAction!default.jspa
        locust.get(
            f'/secure/AjaxIssueEditAction!default.jspa?decorator=none&issueId='
            f'{issue_id}&_={timestamp_int()}',
            catch_response=True)
...
def view_dashboard(locust):
    ...
    content = r.content.decode('utf-8')
    if not (
            f'title="loggedInUser" value="{locust.session_data_storage["username"]}">' in content):
        logger.error(
            f'User {locust.session_data_storage["username"]} authentication failed: {content}')
    assert f'title="loggedInUser" value="{locust.session_data_storage["username"]}">' in content, \
        'User authentication failed'

    # 615 /rest/gadget/1.0/issueTable/jql
    locust.get(
        '/rest/gadget/1.0/issueTable/jql?num=10&tableContext=jira.table.cols.dashboard&addDefault=true'
    ...
    r = locust.post(
        ...
        params={
            'uri': f'{JIRA_SETTINGS.server_url.lower()}//secure/Dashboard.jspa'},
        headers=TEXT_HEADERS,
        catch_response=True)
...
def add_comment(locust):
    ...
    def add_comment_open_comment():
        ...
        r = locust.get(
            f'/secure/AddComment!default.jspa?id={issue_id}',
            catch_response=True)
    ...
    def add_comment_save_comment():
        ...
        r = locust.post(
            f'/secure/AddComment.jspa?atl_token={locust.session_data_storage["token"]}',
            params={
                "id": {issue_id},
                "formToken": locust.session_data_storage["form_token"],
                "dnd-dropzone": None,
        ...
        content = r.content.decode('utf-8')
        if not (
                f'<meta name="ajs-issue-key" content="{issue_key}">' in content):
...
def browse_projects(locust):
    ...
    r = locust.get(
        f'/secure/BrowseProjects.jspa?selectedCategory=all&selectedProjectType=all&page={page}',
        catch_response=True)

    content = r.content.decode('utf-8')
    if not (
            'WRM._unparsedData["com.atlassian.jira.project.browse:projects"]="' in content):
...
def browse_boards(locust):
    ...
    locust.get(
        f'/rest/greenhopper/1.0/rapidviews/viewsData?_{timestamp_int()}',
        catch_response=True)
...
def kanban_board(locust, board_id):
    ...
    project_plan = fetch_by_re(
        params.project_plan_pattern,
        content,
        group_no=2)
    if project_plan:
        project_plan = project_plan.replace('\\', '')
    logger.locust_info(
        f"{params.action_name}: key = {project_key}, id = {project_id}, plan = {project_plan}")

    assert re.compile(f'currentViewConfig.*id.*{board_id}').search(
        content), f'Could not open board with id {board_id}'
    ...
    if project_key:
        ...
        locust.get(
            f'/rest/api/2/project/{project_key}?_={timestamp_int()}',
            catch_response=True)
        ...
        locust.get(
            f'/rest/greenhopper/1.0/xboard/toolSections?mode=work&rapidViewId={board_id}'
            f'&_={timestamp_int()}', catch_response=True)

        # 1050 /rest/greenhopper/1.0/xboard/work/allData.json
        locust.get(
            f'/rest/greenhopper/1.0/xboard/work/allData.json?rapidViewId={board_id}',
            catch_response=True)
...
def scrum_board(locust, board_id):
    ...
    r = locust.get(
        f'/secure/RapidBoard.jspa?rapidView={board_id}',
        catch_response=True)
